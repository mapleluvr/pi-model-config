...
function parseJournal(bytes: Uint8Array): TransactionJournalV1 {
  ...
  if (
    ...
    [record.nativeBeforeHash, record.nativeAfterHash, record.payloadBeforeHash, record.payloadAfterHash].some((hash) => typeof hash !== "string" || !hash.startsWith("sha256:"))
  ) throw new Error("invalid transaction journal");
  return {
    version: 1,
...
export function readCoordinatedSnapshot(options: PayloadCoordinatorOptions = {}): CoordinatedSnapshot {
  ...
  return {
    agentDir,
    native: parsed(read(nativePath), nativePath, (bytes) => parseModelsDocument(nativePath, bytes), () => ({ providers: {} })),
    payload: parsed(read(payloadPath), payloadPath, (bytes) => parsePayloadDocument(bytes, payloadPath), emptyPayloadDocument),
    journal: parsed(read(journalPath), journalPath, parseJournal, () => undefined as never),
  };
}
...
function stableSnapshot(options: PayloadCoordinatorOptions): CoordinatedSnapshot | undefined {
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    const journalOne = read(journalPath);
    const nativeOne = read(nativePath);
    const payloadOne = read(payloadPath);
    const nativeTwo = read(nativePath);
    const payloadTwo = read(payloadPath);
    const journalTwo = read(journalPath);
...
function assertAndRemove(lock: MutationLockHandle, filePath: string): void {
  lock.assertOwned();
  atomicRemove(filePath);
}

function assertAndQuarantine(lock: MutationLockHandle, filePath: string): void {
  lock.assertOwned();
...
function automaticRecovery(lock: MutationLockHandle, snapshot: CoordinatedSnapshot): boolean {
  ...
  const payloadHash = snapshot.payload.parseState === "malformed" && snapshot.payload.exists
    ? (assertAndQuarantine(lock, snapshot.payload.path), hashArtifact(undefined))
    : snapshot.payload.hash;
  assertAndReplace(lock, snapshot.payload.path, serializePayloadDocument(replacement), payloadHash, 0o600);
  assertAndRemove(lock, snapshot.journal.path);
  return true;
}
...
export async function inspectRecovery(options: PayloadCoordinatorOptions = {}): Promise<RecoveryResult> {
  ...
  try {
    const snapshot = readCoordinatedSnapshot({ ...options, agentDir });
...
export async function applyRecovery(snapshotToken: string, choice: RecoveryChoice, options: PayloadCoordinatorOptions = {}): Promise<ApplyRecoveryResult> {
  ...
  try {
    ...
    if (choice === "restore-before-payload" || choice === "restore-after-payload") {
      ...
      const payloadHash = snapshot.payload.parseState === "malformed" && snapshot.payload.exists
        ? (assertAndQuarantine(acquired.handle, snapshot.payload.path), hashArtifact(undefined))
        : snapshot.payload.hash;
      assertAndReplace(acquired.handle, snapshot.payload.path, serializePayloadDocument(payload), payloadHash, 0o600);
      assertAndRemove(acquired.handle, snapshot.journal.path);
      return { type: "recovered" };
    }
    if (choice === "accept-current") {
      assertAndQuarantine(acquired.handle, snapshot.journal.path);
      return { type: "recovered" };
    }
    if (snapshot.journal.parseState === "malformed" && snapshot.journal.exists) assertAndQuarantine(acquired.handle, snapshot.journal.path);
    if (snapshot.payload.exists) assertAndQuarantine(acquired.handle, snapshot.payload.path);
...
export async function commitCoordinatedMutation(request: MutationRequest, options: PayloadCoordinatorOptions = {}): Promise<CommitResult> {
  ...
  if (acquired.type !== "acquired") return acquired;
  try {
    let snapshot = readCoordinatedSnapshot({ ...options, agentDir });
    ...
    const candidate = request.build(snapshot);
    const nativeBytes = serializeModelsDocument(candidate.native);
    const payloadBytes = serializePayloadDocument(candidate.payload);
    ...
    if (nativeChanged && payloadChanged) {
      const journal: TransactionJournalV1 = {
        ...
        beforePayload: snapshot.payload.document!,
        afterPayload: candidate.payload,
      };
      assertAndReplace(acquired.handle, snapshot.journal.path, serializeJournal(journal), snapshot.journal.hash, 0o600);
