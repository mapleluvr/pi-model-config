...
- [ ] Run the exact real owner/contender matrix `Node/Node`, `Node/Bun`, `Bun/Node`, and `Bun/Bun` when Bun is available: owner enters a fixed block, first contender returns typed `busy` and disconnects, owner emits `RESUMED` without uncaught error, second contender still returns `busy`, owner clean release completes, then the next contender returns `acquired`. A separate owner-kill case must also permit immediate `acquired` with no cleanup step.
